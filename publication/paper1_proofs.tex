...
\begin{enumerate}
    ...
    \[
    q_k^{1-\alpha} = \text{Quantile}\Big(\{r_i \colon c_i = k\},\; \frac{\ceil{(n_k+1)(1-\alpha)}}{n_k + 1}\Big)
    \]
    where $\text{Quantile}(\cdot, p)$ returns the $p$-th empirical quantile with interpolation method ``higher'' on the $n_k$ calibration residuals (i.e., the value at index $\ceil{p \cdot n_k}$ in sorted order).
\end{enumerate}
...
\begin{itemize}
    \item Step 3: \texttt{StandardScaler} $\hookrightarrow$ line~158; \texttt{KMeans.fit} $\hookrightarrow$ line~159.
    \item Step 4 — quantile computation: $q_k^{1-\alpha}$ is computed at lines~227--228, 243--244, and 261--262 with \texttt{np.quantile(..., method='higher')}.
    \item Finite-sample correction $\ceil{(n_k+1)(1-\alpha)}/(n_k+1)$ is implemented at those lines.
    \item The quantile level $p_k = \min\big(\ceil{(n_k+1)(1-\alpha)}/(n_k+1),\; 1.0\big)$ is clipped to $[0,\,1.0]$. When $p_k = 1.0$ (equivalently $\ceil{(n_k+1)(1-\alpha)} = n_k+1$), the threshold equals $\max(\text{cal residuals})$ and coverage degrades to $n_k/(n_k+1)$; this occurs when $n_k < \ceil{1/\alpha} - 1$.
\end{itemize}
\end{remark}

\begin{lemma}[Finite-Sample Correction Monotonicity]\label{lem:correction}
For $n_k \geq 1$ and $\alpha \in (0,1)$, let $m = \ceil{(n_k+1)(1-\alpha)}$ as in the code. The effective quantile level is $p_k = \min(m/(n_k+1),\; 1.0)$. When $m \leq n_k$ (equivalently $n_k \geq \ceil{1/\alpha} - 1$), the coverage guarantee is:
\[
\Pbb(r_{\text{new}} \leq q_k^{1-\alpha}) = \frac{m}{n_k + 1} \;\geq\; 1 - \alpha.
\]
When $m = n_k + 1$ (i.e., $p_k = 1.0$ and $n_k < \ceil{1/\alpha} - 1$), the threshold is $\max(\text{cal residuals})$ and coverage equals $n_k/(n_k+1)$, which may fall below $1-\alpha$.
\end{lemma}
\begin{proof}
The coverage bound follows from the rank-uniformity argument in Theorem~\ref{thm:coverage}. When $m \leq n_k$, the event $\{r_{\text{new}} \leq q_k^{1-\alpha}\}$ is exactly $\{\text{rank}(r_{\text{new}}) \leq m\}$, giving probability $m/(n_k+1) \geq 1-\alpha$ by the ceiling property. When $m = n_k+1$, the quantile at level~$1.0$ equals the maximum calibration residual, so coverage is $n_k/(n_k+1)$. The condition $m \leq n_k$ is equivalent to $\ceil{(n_k+1)(1-\alpha)} \leq n_k$, which requires $n_k \geq \ceil{1/\alpha} - 1$. The clipping logic is verified at lines~227, 243, and 261; the small-cluster warning at lines~209--213.
\end{proof}
...
\textbf{Step 2 — Coverage event.} Let $m = \ceil{(n_k+1)(1-\alpha)}$. The quantile $q_k^{1-\alpha}$ is computed at the effective level $\min(m/(n_k+1),\; 1.0)$ (Lemma~\ref{lem:correction}) via \texttt{np.quantile(..., method='higher')}. By assumption (A3), $n_k \geq \ceil{1/\alpha} - 1$, which implies $m \leq n_k$ and $p_k < 1.0$. Under this condition, the event $\{r_{\text{new}} \leq q_k^{1-\alpha}\}$ is exactly the event that $r_{\text{new}}$ has rank at most $m$ in the combined set $\mathcal{R}_k$.
...
The bound is exact when $\ceil{(n_k+1)(1-\alpha)} = (n_k+1)(1-\alpha)$, i.e., when $(n_k+1)(1-\alpha)$ is integer. For $n_k \geq \ceil{1/\alpha} - 1$ (condition A3), the excess coverage above $1-\alpha$ is at most $1/(n_k+1)$. The code issues a warning at $n_k < 1/\alpha$ (lines~176--179), which is slightly more conservative than the theoretical threshold $n_k \geq \ceil{1/\alpha} - 1$ (difference at most 1 sample). Below this threshold, coverage degrades to $n_k/(n_k+1)$, which may fall below $1-\alpha$.
\end{remark}

% ------------------------------------------------------------------------------

\begin{corollary}[Simultaneous Coverage via Max Aggregation]\label{cor:simultaneous}
Using the \texttt{multivariate\_strategy='max'} option (lines~88--89), the per-sample calibration score is $r_i = \max_{h,d} |y_i^{h,d} - \hat{y}_i^{h,d}|$. Then Theorem~\ref{thm:coverage} yields simultaneous coverage across all $H \cdot D_{\text{out}}$ forecast elements:
\[
...
\]
Since $\Pbb_{\text{cal}}(r_{\text{new}} \leq q_k) \geq 1-\alpha$, the bound follows under the stated covariate-shift assumptions. In the current implementation, test samples are assigned to the nearest fitted K-Means cluster; no distance-based soft fallback is implemented.
\end{proof}
...
\]
where $C_k = O(\sigma_k \cdot \sqrt{\log(1/\alpha)})$ depends on the residual tail. For $|\rho_k| \leq 0.3$ (the correction threshold at line~187), the coverage deficit from autocorrelation is $\leq 0.03$ for typical $n_k \geq 20$.
\end{theorem}
\begin{proof}
...
The K-Means clustering $\kappa$ is fit on the same calibration data used for quantile estimation (lines~158--159). Lemma~\ref{lem:cluster-exchangeable} proves that this is valid: since $\kappa$ is a deterministic function of $\{\mathbf{s}_i = f_\theta(\mathbf{x}_i)\}$ only — i.e., a $\sigma(\{\mathbf{x}_i\})$-measurable partition — the within-cluster residuals are conditionally exchangeable given the partition. No auxiliary clustering set is required. The \texttt{check\_exchangeability()} method (line~261) provides an empirical diagnostic for the time-series case: it computes lag-1 autocorrelation within each cluster and reports any applied ACF-inflation correction factors. Values $|\text{ACF}(1)| \leq 0.3$ suggest approximate exchangeability; Theorem~\ref{thm:autocorrelation} bounds the coverage deficit when stronger autocorrelation is present.

\subsection*{ACF-Aware Quantile Correction (Theorem~\ref{thm:autocorrelation} Implementation)}

When the constructor parameter \texttt{correct\_acf=True} (default), the \texttt{fit()} method automatically corrects for residual autocorrelation at lines~185--196. For each cluster, after computing the base quantile $q_k^{\text{base}}$, the lag-1 autocorrelation $\rho_k$ is computed via \texttt{\_compute\_acf1()}. If $|\rho_k| > 0.3$, the effective sample size $n_k^{\text{eff}} = n_k \cdot (1-|\rho_k|)/(1+|\rho_k|)$ and the standard-error inflation factor $\sqrt{n_k / n_k^{\text{eff}}} = \sqrt{(1+|\rho_k|)/(1-|\rho_k|)}$ are used to inflate the quantile:
\[
q_k = q_k^{\text{base}} \cdot \Big(1 + \frac{\sqrt{(1+|\rho_k|)/(1-|\rho_k|)} - 1}{\sqrt{n_k}}\Big).
\]
The correction factors are stored in \texttt{self.acf\_corrections\_} and reported by \texttt{check\_exchangeability()}. This implements a conservative ACF-aware inflation heuristic motivated by Theorem~\ref{thm:autocorrelation}; it should be validated empirically rather than presented as an unconditional restoration of the $1-\alpha$ guarantee.
...
\begin{itemize}
    ...
    \item \textbf{Assumption (A2) — Exchangeability:} For time-series data, residuals within a state cluster exhibit temporal autocorrelation. Theorem~\ref{thm:autocorrelation} bounds the coverage deficit as $O(|\rho_k|/\sqrt{n_k^{\text{eff}}})$, providing a quantitative degradation guarantee. The \texttt{check\_exchangeability()} diagnostic (line~261) computes ACF(1); values $|\text{ACF}(1)| \leq 0.3$ imply a coverage deficit $\leq 0.03$ for $n_k \geq 20$. \textbf{Risk level:} MODERATE (quantitatively bounded).
...
\begin{itemize}
    \item Distribution shift (concept drift, regime change) is bounded by Corollary~\ref{cor:distribution-shift}: coverage degrades by at most $\text{TV}(P_{\text{cal}}, P_{\text{test}})$ under covariate-shift assumptions. New regimes not represented in calibration remain a practical failure mode.
\end{itemize}
...
\textbf{Overall evidence quality:} MODERATE until the full empirical validation suite is complete. Lemma~\ref{lem:cluster-exchangeable} resolves the clustering-information-leakage concern under split independence. Theorem~\ref{thm:autocorrelation} motivates diagnostics for temporal dependence. Corollary~\ref{cor:simultaneous} supports simultaneous multivariate coverage via the \texttt{max} strategy under its assumptions. Remaining limitations are the cluster-size condition (A3), approximate exchangeability in time series, and distribution shift.
...
where:
\begin{itemize}
    \item $A(\theta) \in \R^{5 \times 5}$ is block-diagonal with learned parameters $\theta = (\alpha_L, \alpha_T, \gamma, \omega, \alpha_R)$, parameterized via sigmoid gates (encoder lines~74--80):
    \[
    A = \begin{bmatrix}
    \alpha_L & 0       & 0        & 0        & 0      \\
    0        & \alpha_T& 0        & 0        & 0      \\
    ...
    \item $B(\mathbf{x}_t) = W_B \cdot \text{GELU}(\text{LayerNorm}(W_{\text{proj}}\mathbf{x}_t + b_{\text{proj}})) + b_B$ is the innovation term (encoder lines~36--40, applied at line~97).
    \item $\beta = \texttt{softplus}(\beta_{\text{raw}}) \in \R^+$ is a learnable scalar initialized to yield $\beta = 0.01$ (line~48). The softplus gate ensures $\beta \geq 0$. Each linear layer of the correction MLP is spectrally normalized (\texttt{nn.utils.spectral\_norm}) guaranteeing $\norm{J_{\text{MLP}}}_2 \leq 1$. The $\tanh$ ensures $\norm{\text{correction}}_\infty \leq \beta$.
\end{itemize}
\end{definition}
...
The structured dynamics guarantee that the forward state does not diverge during training. The eigenvalues of $A$ are bounded at $\leq 1$ by the sigmoid gates. With spectral normalization applied to the correction MLP (Theorem~\ref{thm:gradient-stability}), the per-step Jacobian is bounded by $1 + L_G\beta$ where $L_G \approx 1.77$ is the GELU Lipschitz constant, providing a valid bound on gradient magnitudes. At $\beta \approx 0.01$ (softplus initialization), the per-step bound is $\approx 1.0177$. Note: computing $(1.0177)^{720} \approx 2.5\times 10^5$ gives a worst-case cumulative growth ceiling; in practice, gradient norms are controlled by the structured eigenvalues of $A$ being $\leq 1$. The BIBO stability also ensures that the encoder generalizes to input lengths not seen during training, provided the innovation function $B(\cdot)$ is bounded (guaranteed by the LayerNorm + GELU activation chain at lines 36--39).
\end{remark}
...
\;\leq\; 1 + L_G \beta \approx 1 + 1.77\beta.
\]
For sequence length $L$, the gradient norm is controlled by the recurrent Jacobian and correction scale. The implementation supports gradient clipping and optional correction-scale regularization; these engineering controls should be reported alongside any long-horizon stability claims.
\end{theorem}
\begin{proof}
Spectral normalization constrains each linear layer $W$ to satisfy $\norm{W}_2 \leq 1$ (Miyato et al., 2018). GELU is not 1-Lipschitz: $L_G = \sup_x |\operatorname{GELU}'(x)| \approx 1.77$. For a two-layer MLP with GELU activation, $\norm{J_{\text{MLP}}}_2 \leq \norm{W_2}_2 \cdot L_G \cdot \norm{W_1}_2 \leq L_G \approx 1.77$. The $\tanh$ derivative satisfies $\norm{\operatorname{diag}(1-\tanh^2)}_2 \leq 1$ element-wise. The softplus gate ensures $\beta \geq 0$. Differentiating the recurrence (Definition~\ref{def:transition}):
\[
...
The \texttt{apply\_structured\_A} method (lines~82--94) exactly multiplies the block-diagonal matrix $A$ from Definition~\ref{def:transition}. The seasonal rotation at lines~91--92 computes:
\begin{align*}
...
The correction term clips the MLP output via $\tanh$ and scales by $\beta = \texttt{softplus}(\beta_{\text{raw}})$ (initialized to yield $\beta \approx 0.01$). Since $\tanh(\cdot) \in (-1, 1)^5$, $\norm{\text{correction}}_\infty \leq \beta$ element-wise. Spectral normalization constrains the linear layers, but the full nonlinear Jacobian still depends on activations and composition, so the implementation also exposes gradient clipping and optional correction-scale regularization.

\subsection*{Innovation Boundedness Verification}

The innovation term at line~97: $B(\mathbf{x}_t) = W_{\text{innovation}} \cdot \text{LayerNorm}(\text{GELU}(W_{\text{proj}}\mathbf{x}_t + b_{\text{proj}})) + b_{\text{innovation}}$. The GELU and LayerNorm are both Lipschitz-continuous and bounded on bounded inputs. For normalized data (standard scaling applied in the data loader), $\norm{\mathbf{x}_t}_\infty$ is bounded, so $B(\mathbf{x}_t)$ is uniformly bounded. This confirms the BIBO stability claim.
