...
- **Novel, and regime-dependent**: state-conditional interval calibration. The conditioning signal is a **state x cell interaction** — the state changes *which horizon-feature cells are hard*, not how hard a window is overall. Per-cluster quantile *shape* deviates by ~0.20-0.26 while per-cluster *level* varies by only ~0.03, and the per-sample axis holds under 1.2% of residual variance against ~19% on the per-cell axis. Consequences: a **scalar** `sigma(s)` cannot express the signal (measured +0.011 Winkler vs flat CP, 5/12 seed-cut wins — no improvement, and a scalar label-informed per-sample reference using test labels gains under 1%), whereas `StateScaledConformal(scale_geometry='per_cell')`, one log-linear regression per cell, reaches -0.237 (3 seeds x 4 nested cuts, better on all 12) and cluster SCCP reaches -0.124 (same, better on all 12). The cuts are nested (`[cut*n:]`), so this is not 12 independent trials — effective n is closer to 3 seeds, and the seeds share one dataset/split/test-row set, so a population-level claim has an effective n closer to 1 cell. All of this holds **only in the RevIN regime**; on a pre-RevIN (amplitude-collapsed) run every mechanism loses to flat CP. Never state a conditioning result without the regime. The cluster-vs-per-cell ordering above was measured under an earlier, asymmetric fitting scheme (see below) and has not yet been re-measured under the fixed one — do not treat it as current evidence.
- **Best-scoring so far, but measured under a now-fixed asymmetry — needs re-measurement**: `StateConditionalConformal` (cluster) scored highest on ETTh1-h336/seed 42/RevIN in the last measurement, paired on identical residuals: flat 3.7869, state-scaled scalar 3.7877, state-scaled per-cell 3.6916, cluster 3.5962. Per-cell state-scaled beat flat CP and the scalar geometry; it did not beat cluster SCCP in that table. But that table was produced while cluster SCCP's conditioning fit saw ~9x more data than state-scaled CP's (train states vs. half the calibration split) — `run_benchmark.py:754-761` now fits both mechanisms on the same calibration-half `conditioning_states`, removing that asymmetry, so the ordering above must be re-run under the current code before being cited. Do not call either mechanism the paper's established primary contribution on ETTh1 alone.
- **Open, not measured**: the protocol result trails the diagnostic and the cause is not established. `sigma(s)` is *not* fit on train residuals — `fit_scale` receives the first calibration half (`run_benchmark.py:754-761`), so it is already out-of-sample with respect to the forecaster's training data. The likeliest remaining explanation is fitting-set size (~696 windows under the protocol vs. the diagnostic's larger window), but no run has varied that while holding everything else fixed. An earlier version of this bullet quoted a 0.076-vs-0.137 in-sample/out-of-sample coefficient spread as a measured limitation; that was withdrawn together with the train-residual premise it rested on. Quote no spread number until a run measures one.
...
- `StateConditionalConformal.fit_partition(...)` and `StateScaledConformal.fit_scale(...)` must each precede their own `calibrate(cal_states, residuals)`. Both are fit on the **same** window: the first half of the calibration split (`conditioning_states`/`conditioning_residuals`, via `_split_calibration_indices` in `run_benchmark.py`), not on train states — this equalises the two mechanisms' fitting-set size and in-sample status (an earlier version fit the cluster partition on train states instead, a ~9x asymmetry; see `docs/methodology.md` for that history and why prior cluster-vs-scale orderings measured under it need re-running). Do not fit either mechanism on the calibration quantile half or on test data.
...
- Conditional-coverage claims (as opposed to marginal PICP) require `conditional_coverage.worst_slab_coverage` / `max_coverage_deviation` in `metrics.json`, computed on prespecified, method-agnostic bins (`fit_coverage_bin_edges`/`conditional_coverage_by_bin` in `cissn/evaluation/metrics.py`) — never on each method's own partition, which would make the comparison unfair. `run_benchmark.py` fits these edges on the calibration-half `conditioning_states`, the same window `fit_partition`/`fit_scale` use, not on train states; `tests/test_experiment_runners.py::TestConditioningCalibrationDataSource` pins this call site.
