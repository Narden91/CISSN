...
- Every benchmark run calibrates flat CP on the same residuals from the same model and scores it on the same forecasts, writing `interval_flat_cp` to `metrics.json`. This is the paired evidence for state conditioning; never compare SCCP against a flat CP fitted in a separate training run.
