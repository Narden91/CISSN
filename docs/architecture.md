...
For intervals, the trained model is frozen. `--conformal_conditioning` selects the primary state conditioning mechanism, calibrated on the later chronological calibration split; it never refits during calibration or test evaluation. The two mechanisms' conditioning-fit windows differ: `cluster` fits its partition on train-split states, `scale` fits its sigma regression on the first half of the calibration split — see `docs/methodology.md` for the resulting asymmetry:
...
- `cluster` (`StateConditionalConformal`): learns a K-Means partition of the state and calibrates one quantile per cluster, so each cluster gets its own full quantile *surface* — which is why it captures the same state x cell interaction. Sparse calibration clusters use the global calibration quantile and are recorded in `cluster_stats.json`. This is the default primary mode and currently scores best on ETTh1-h336, but that ordering is confounded by a ~9x conditioning-fit sample-size asymmetry against state-scaled CP (cluster fits on train states, state-scaled fits on half the calibration split) and is not yet a fair comparison — see `docs/methodology.md`.
