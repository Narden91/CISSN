    ...
    if lower is not None and upper is not None:
        ...
        if y_train is not None:
            ...
            msis_val = mean_scaled_interval_score(
                lower, upper, trues, y_train, seasonal_period, alpha=args.conformal_alpha
            )
...
def run_point_baseline(args, setting: str):
    ...
    if args.model == "deepstate":
        # DeepState's intervals come from its own parametric predictive
        # distribution, not from split-conformal calibration on held-out
        # data. Label them distinctly so they are never averaged into a
        # coverage table alongside conformalized methods.
    ...
    point_metrics, interval_metrics = compute_metrics(
        ...
        interval_origin="raw_parametric" if args.model == "deepstate" else "conformalized",
    )
    runtime = {
...
def conformalize_uq_intervals(
    ...
    calibrator = FlatConformal(alpha=alpha, multivariate_strategy="per_feature")
...
def run_mc_dropout(args, setting: str):
    ...
    if args.uq_interval_mode == "conformalized":
        ...
        lower, upper = conformalize_uq_intervals(
            preds, cal_trues, cal_preds, cal_lower, cal_upper, lower, upper, args.conformal_alpha
        )
